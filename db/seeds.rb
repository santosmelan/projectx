...
if User.count == 0
    ...
    member      = User.new(role: 'member', email: 'member@projectxx.com', first_name: 'Juan', last_name: 'Cruz', password: 'password', password_confirmation: "password", created_at: DateTime.now, updated_at: DateTime.now)
    member.save!
    puts "Member account has been created"

    super_admin = User.new(role: 'super_admin', email: 'super_admin@projectxx.com', first_name: 'James', last_name: 'Wayne', password: 'password', password_confirmation: "password", created_at: DateTime.now, updated_at: DateTime.now)
    super_admin.save!
    puts "Super Admin account has been created"

    admin       = User.new(role: 'admin', email: 'admin@projectxx.com', first_name: 'Kit', last_name: 'Santos', password: 'password', password_confirmation: "password", created_at: DateTime.now, updated_at: DateTime.now)
    admin.save!
    puts "Admin account has been created"
end
